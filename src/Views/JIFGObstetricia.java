...
public class JIFGObstetricia extends javax.swing.JInternalFrame {
    private static JIFGObstetricia instancia;
    ...
    private JIFGObstetricia() {
        initComponents();

        
        this.activarCtrls(false);
        this.tablaObstetricia();

        cargarPacientes();
        cargarMedicos(); // Ya trae solo los de Obstetricia
        cargarAreas();   // Solo carga “Obstetricia”
    ...
    public static JIFGObstetricia getInstancia(){
        if(instancia == null || instancia.isClosed()){
            instancia = new JIFGObstetricia();
        }
        return instancia;
    }
...
for (Especialidad e : listaEspecialidades) {
    if (e.getTipoE().equalsIgnoreCase("Obstetricia")) {
        area = e;
        break;
    }
}
    ...
    // ✅ Volver a cargar tabla de OBSTETRICIA
    tablaObstetricia();
    limpiarFormulario();
    activarCtrls(false);
...
if (confirmacion == JOptionPane.YES_OPTION) {
    try {
        ...
        // ✅ ACTUALIZAR TABLA DE OBSTETRICIA (no Enfermería)
        tablaObstetricia();
...
private void tablaObstetricia() {
    try {
        GHistoria bdObstetricia = new GHistoria();
        ArrayList<Historia> arrObstetricia = bdObstetricia.listarPorEspecialidad(6); // ✅ ID de Obstetricia

        if (arrObstetricia != null && !arrObstetricia.isEmpty()) {
            mtE.setListHistoria(arrObstetricia);
            tblLista.setModel(mtE);
            tblLista.repaint();
        } else {
            JOptionPane.showMessageDialog(this, 
                "No se encontraron registros de historias médicas de Obstetricia", 
...
private void cargarMedicos() {
    try {
        ...
        listaMedicos = gm.listarPorEspecialidad(6); // 6 = OBSTETRICIA (ajusta según tu BD)
...
private void cargarAreas() {
    try {
        ...
        listaEspecialidades = ge.listar(); // Guarda todas las especialidades reales

        cmboxArea.removeAllItems(); // Limpiar el combo

        for (Especialidad esp : listaEspecialidades) {
            if (esp.getTipoE().equalsIgnoreCase("Obstetricia")) {
                cmboxArea.addItem(esp.getTipoE()); // Mostrar solo el nombre
            }
        }

        cmboxArea.setEnabled(false); // Desactivarlo para evitar cambios
    } catch (Exception e) {
    ...
    if (!texto.isEmpty()) {
        try {
            ...
            for (Historia h : gh.buscarPorNombreODni(texto)) {
                if (h.getEspecialidad().getId() == 6) { // ✅ Solo Obstetricia
                    resultados.add(h);
                }
            }
        ...
        tablaObstetricia(); // ✅ Recarga todo si no hay texto
    }
}
