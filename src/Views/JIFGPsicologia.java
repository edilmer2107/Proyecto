...
public class JIFGPsicologia extends javax.swing.JInternalFrame {
    private static JIFGPsicologia instancia;
    ...
    private JIFGPsicologia() {
        initComponents();

        
        this.activarCtrls(false);
        this.tablaPsicologia();
    ...
    public static JIFGPsicologia getInstancia(){
        if(instancia == null || instancia.isClosed()){
            instancia = new JIFGPsicologia();
        }
        return instancia;
    }
...
for (Especialidad e : listaEspecialidades) {
    if (e.getTipoE().equalsIgnoreCase("Psicologia")) {
    area = e;
    break;
...
private void tablaPsicologia() {
    try {
        GHistoria bdPsicologia = new GHistoria(); // ✅ nombre correcto
        ArrayList<Historia> arrPsicologia = bdPsicologia.listarPorEspecialidad(5); 

        if (arrPsicologia != null && !arrPsicologia.isEmpty()) {
            mtE.setListHistoria(arrPsicologia);
            tblLista.setModel(mtE);
            tblLista.repaint();
        } else {
            JOptionPane.showMessageDialog(this, 
                "No se encontraron registros de historias médicas de Psicología", 
...
private void cargarMedicos() {
    try {
        ...
        listaMedicos = gm.listarPorEspecialidad(5); 
...
private void cargarAreas() {
    try {
        ...
        listaEspecialidades = ge.listar(); 

        cmboxArea.removeAllItems(); // Limpiar el combo

        for (Especialidad esp : listaEspecialidades) {
           if (esp.getTipoE().equalsIgnoreCase("Psicologia")) {
             cmboxArea.addItem(esp.getTipoE());
        }
